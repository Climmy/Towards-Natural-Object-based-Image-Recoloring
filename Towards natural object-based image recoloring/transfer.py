...
def img_color_transfer(img, original_p, modified_p, sample_weight_map, sample_colors, sample_rate, mask):
...
#	colors = img.getcolors(img.width * img.height)
	...
	for ii in range(img.width):
		for jj in range(img.height):
			if mask[ii,jj] > 0:
				if bins.get(pixels[ii,jj])==None:
					bins[pixels[ii,jj]] = 1
					colors.append(pixels[ii,jj])
	args = []
#	for _,c in colors:
	for c in colors:
...
#		color_map[colors[i][1]] = tuple([int(x) for x in interp_res[i]])
		color_map[colors[i]] = tuple([int(x) for x in interp_res[i]])
	...
	for i in range(img.width):
		for j in range(img.height):
			if mask[i,j]>0:
				t1 = color_map[img_pixels[i,j]][0]*mask[i,j]/255 + img_pixels[i,j][0]*(255-mask[i,j])/255
				t2 = color_map[img_pixels[i,j]][1]*mask[i,j]/255 + img_pixels[i,j][1]*(255-mask[i,j])/255
				t3 = color_map[img_pixels[i,j]][2]*mask[i,j]/255 + img_pixels[i,j][2]*(255-mask[i,j])/255
				result_pixels[i,j] = (int(t1),int(t2),int(t3))
			else:
				result_pixels[i,j] = img_pixels[i,j]
